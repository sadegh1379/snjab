    ...
    const {loadUsers,committees,selectedCommittee,loadCommittees,search,has_more,wards,ward}=this.state;
        return (
            <div >
                ...
                >
                    <div onClick={this.closeModal} className=" container w-100 btn-primary iran-sans_Bold text-center p-2  text-white" style={{fontSize:'13px',borderTopLeftRadius:'20px',borderTopRightRadius:'20px'}}>انتخـاب اعضـا (اعضـای فعـال و عضـو سامانـه سنجـاب) </div>

                    <div className="py-3 bg-white  container" style={{borderBottomRightRadius: '20px',borderBottomLeftRadius: '20px'}}>
                            ...
                            <div className="row">
                                ...
                                <div className="col-lg-7 bg-white pr-0">
                                    <div className="shadow-sm overflow-auto" ref={(ref) => this.scrollParentRef = ref} style={{height:600}}>
                                        <div className="d-flex w-100 bg-dark iran-sans_Bold text-center p-3 text-white" style={{borderTopRightRadius:'20px',borderTopLeftRadius:'20px'}}>
                                            <div className="text-white" style={{width:'10%'}}>
                                                <button className="btn btn-link p-0" style={{fontSize: '22px'}} type={"button"} onClick={()=>{this.toggleChecked()}}>
                                                    <i className={`fal text-white ${this.state.isCheckedAll?'fa-check-square':'fa-square'}`}/>
                                                </button>
                                            </div>
                                            <div className="text-white" style={{width:'45%',fontSize: '13px',paddingTop: '5px',fontFamily:'iransansBold'}}>نـام و نـام خانوادگـی </div>
                                            <div className="text-white" style={{width:'45%',fontSize: '13px',paddingTop:'5px',fontFamily:'iransansBold'}}>سمت</div>
                                        </div>
                                        ...
                                        >
                                            {this.state.users.map(
                                                (user, index) => {
                                                    return (
                                                        <button onClick={()=>this.toggleChecked(user)} className="btn w-100 d-flex text-center" style={{borderBottom: '1px solid rgb(198, 198, 198)',paddingTop: '15px'}} key={index}>
                                                            <div className=" text-dark" style={{width:'10%',fontSize:'22px'}}>
                                                               <i className={`fal ${this.userChecked(user.id)?'fa-check-square text-success':'fa-square'}`}/>
                                                            </div>
                                                            <div className=" text-dark" style={{width:'45%',fontSize: '13px'}}> {user.firstname+' '+user.lastname}</div>
                                                            <div className=" text-dark" style={{width:'45%',fontSize: '13px'}}>{user.post || '-'}</div>
                                                        </button>
                                                    )
                                                }
                                        ...
                                        {!loadUsers && this.state.users.length==0 && <p className=" text-center h5 py-5" style={{fontSize: '13px',fontFamily: 'iransansbold',color: 'rgba(242,7,7,0.56)'}}>برای جستجـو شمـا اطلاعـاتـی نیافتیـم</p>}
                                    </div>
                                </div>
                            </div>
                            <br />
                            <div className="row justify-content-center pb-4 mt-3">
                                <button type="button" className="btn btn-blue rounded-pill mx-4 my-1 col-sm-2"  style={{fontFamily:'iransansbold'}} onClick={this.submit}>ثبت </button>
                                <button type="button" onClick={this.closeModal} className="btn btn-outline-blue rounded-pill mx-4 my-1 col-sm-2" style={{fontFamily:'iransansbold'}}>انصـراف</button>
                            </div>
                        </div>
                    </div>
