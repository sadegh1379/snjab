...
import { ChecklistState } from "./Pages/checklist/listchecklist";

import { Education } from "./Pages/pdp";
...
import { OldIndicatorCollectorOrMonitor } from "./Pages/indicator/OldIndicatorCollectorOrMonitor";
import { IndicatorCollctorCompact } from "./Pages/indicator/IndicatorCollctorCompact";
...
class App extends Component {
  constructor(props) {
    ...
    axios.defaults.headers.common["Authorization"] =
      this.props.globalStorage.token;
    ...
    return !this.state.waitToRender ? (
      <Router history={history}>
        <div style={{ minHeight: "100vh" }}>
          ...
          <Route
            path="/updateUsers/:hospital/:password"
          ...
          <PrivateRoute
            path="/indicator"
            component={IndicatorIndexComponent}
            exact
          />
          <PrivateRoute
            path="/indicator/safety_related"
            component={SafetyRelatedIndicatorComponent}
            exact
          />
          <PrivateRoute
            path="/indicator/dashboard"
            component={IndicatorDashbaord}
            exact
          />
          <PrivateRoute
            path="/indicator/monitor"
            component={OldIndicatorCollectorOrMonitor}
            exact
          />
          <PrivateRoute
            path="/indicator/collector"
            component={NewIndicatorCollectorOrMonitor}
            exact
          />
          <PrivateRoute
            path="/indicator/collector/compact"
            component={IndicatorCollctorCompact}
            exact
          />
          <PrivateRoute
            path="/indicator/list"
          ...
          />
          <PrivateRoute
            path="/indicator/list/edit/:id"
            component={IndicatorCreateComponent}
            exact
          />
          <PrivateRoute
            path="/indicator/list/checklist/:id"
            component={IndicatorChecklistComponent}
            exact
          />
          {/* <PrivateRoute path="/checklist/" component={ChecklistCreate} exact/> */}
          <PrivateRoute
            path="/checklist/"
            component={CheckListIndexComponent}
            exact
          />
          <PrivateRoute
            path="/rezayat/"
          ...
          />
          <PrivateRoute
            path="/checklist/checklist_page"
            component={CheckListPageComponent}
            exact
          />
          <PrivateRoute
            path="/checklist/dashboard"
            component={CheckListDashbaord}
            exact
          />
          <PrivateRoute
          ...
          {globalStorage && globalStorage.loading > 0 && (
            <div
              className="loading_req position-fixed bg-white w-100 h-100 zoomIn"
              style={{ top: 0, left: 0, zIndex: 99999 }}
            >
              <img
                src={loading}
                alt="در حال دریافت اطلاعات..."
                className="position-absolute m-auto loading"
        ...
        <div className="wave">
          ...
          >
            <i
              className="fal fa-chevron-up  circle"
              style={{
                background: "#104c82",
                padding: "10px",
                color: "#ffffff",
          ...
          <p className="text-center" style={{ fontSize: "0.8em" }}>
            ...
            <a
              href="https://noavaran-sharif.ir"
              className="px-1 text-center"
              style={{ color: "#104c82" }}
            >
              {" "}
              نـوآوران سلامت گستر شریف{" "}
            </a>
            است
          </p>
        </div>
